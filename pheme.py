...
# import torch.nn.init as init  初始化神经网络中的权重
from sklearn.model_selection import KFold  # 实现 K 折交叉验证
from sklearn.model_selection import train_test_split  # 将数据集划分为训练集和测试集
from torch.optim.lr_scheduler import StepLR  # StepLR 学习率调度器，用于在训练过程中调整学习率
import torch.optim as optim  # 导入 torch.optim 模块并重命名为 optim，用于优化器的实现
from os import path
from torch.autograd import Variable  # 包装张量，以便进行自动微分
import math
from torch_position_embedding import PositionEmbedding  # 用于添加位置编码（通常用于处理序列数据）
import random
# from pheme_data_get import data_get
import os
import random
import json
import scipy.sparse as sp  # 导入 scipy.sparse 模块并重命名为 sp，用于处理稀疏矩阵
import torch.nn.functional as F  #  导入 PyTorch 的 torch.nn.functional 模块并重命名为 F，用于激活函数和其他功能
import torch.nn.init as init  # 用于初始化神经网络中的权重
import time
...
def data_loader(pathway='../processed_data/PHEME/pheme_n5_p5_u100/normalized_news_nodes/', node_type="news"):
    if node_type == "news":
        news_node = []
        news_id = []
        news_label = []
...
with open("../rwr_results/pheme_n5_p5_u100/original_adj",'r',encoding='utf-8') as f:
    adj_list=json.load(f)

neighbor_dict = neighbor_loader('../rwr_results/pheme_n5_p5_u100/n_neighbors.txt')
post_nodes = data_loader(pathway='../processed_data/PHEME/pheme_n5_p5_u100/normalized_post_nodes/',
    node_type="post")
news_nodes = data_loader(pathway='../processed_data/PHEME/pheme_n5_p5_u100/normalized_news_nodes/',
                         node_type="news")
user_nodes = data_loader(pathway='../processed_data/PHEME/pheme_n5_p5_u100/normalized_user_nodes/',
    node_type="user")
...
class DTN(nn.Module):
    def __init__(self, input_dim, n_hidden_dim, u_hidden_dim, p_hidden_dim, out_embed_d, outemb_d=1,
                 content_dict={}, attn_heads=8, d_model=104, self_attn_heads=8, enc_layers=1, dec_layers=1):
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n, all_t0,npu=15,n=5):
        ...
        self_att_c_all = self.mh_attention1(encoder_emb_input.view(bsz_all, -1, self.hidden_dim),encoder_emb_input.view(bsz_all, -1, self.hidden_dim), encoder_emb_input.view(bsz_all, -1,self.hidden_dim))  # [1+n_nei_num+p+u,1,H]
...
five_res = []
base_PATH = '../pre-trained/PHEMEall/'
if not os.path.exists(base_PATH):
    os.makedirs(base_PATH)
f_path = base_PATH + 'result.txt.txt'
for repeat in range(5):
    ...
    for fold in range(start_fold,num_folds):
        ...
        test_loss = 0.0
        best_epoch = 0
        net = DTN(input_dim=[768,512, 6], n_hidden_dim=hidden_dim, u_hidden_dim=hidden_dim, p_hidden_dim=hidden_dim,
    ...
    for k in range(len(test_set)):
        ...
        for fold in range(num_folds):
            all_t = []
            all_t.append(neighbor_dict[2][test_set[k].node_id])
            all_t.extend(neighbor_dict[3][neighbor_dict[2][test_set[k].node_id]])
            result,dist_og  = best_models[fold](test_set[k], neighbor_dict[0][test_set[k].node_id],
                                       neighbor_dict[1][test_set[k].node_id],all_t)
            output += result.item()

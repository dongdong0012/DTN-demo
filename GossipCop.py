...
p_add_time_dict = {}
n_add_time_dict = {}
with open("../rwr_results/fnn_gossipcop_u50/n_add_time.txt", 'r') as f:
    for line in f:
        line = line.strip()
        if line:
            key, value = line.split(' ', 1)
            n_add_time_dict['gossipcop-' + key] = value
with open("../rwr_results/fnn_gossipcop_u50/p_add_time.txt", 'r') as f:
    for line in f:
        line = line.strip()
        if line:
            key, value = line.split(' ', 1)
...
neighbor_dict = neighbor_loader('../rwr_results/fnn_gossipcop_u50/n_neighbors.txt')

# with open("../../0data/FakeNewsNet-Dataset/FakeNewsNet_Dataset/graph_def/gossipcop/original_adj",'r') as f:
with open("../rwr_results/fnn_gossipcop_u50/original_adj", 'r') as f:
...
post_nodes = data_loader(pathway='../processed_data/FakeNewsNet/GossipCop/fnn_gossipcop_50/normalized_post_nodes/',
                         node_type="post")
user_nodes = data_loader(pathway='../processed_data/FakeNewsNet/GossipCop/fnn_gossipcop_50/normalized_user_nodes/',
                         node_type="user")
news_nodes = data_loader(pathway='../processed_data/FakeNewsNet/GossipCop/fnn_gossipcop_50/normalized_news_nodes/',
                         node_type="news")
...
class Signed_GAT(nn.Module):
    ...
    def forward(self, node_embedding, cosmatrix, original_adj, X_tid):
        ...
        potentinal_adj = torch.where(cosmatrix > 0.5, torch.ones_like(cosmatrix), torch.zeros_like(cosmatrix)).to(
            device)
        adj = original_adj + potentinal_adj
        adj = torch.where(adj > 0, torch.ones_like(adj), torch.zeros_like(adj))

        self.attentions = [
            GraphAttentionLayer(embedding_dim, embedding_dim, dropout=self.dropout, alpha=self.alpha, concat=True) for _
            in
            range(self.nb_heads)]
        for i, attention in enumerate(self.attentions):
            self.add_module('attention_{}'.format(i), attention)
        out_att = GraphAttentionLayer(embedding_dim * self.nb_heads, embedding_dim, dropout=self.dropout,
                                      alpha=self.alpha,
                                      concat=False)
...
class DTN(nn.Module):
    def __init__(self, input_dim, n_hidden_dim, u_hidden_dim, p_hidden_dim,
        ...
        self.entropy_data = self.load_entropy_data()
    ...
    def Bi_RNN(self, neighbor_id, neighbor_time, node_type, post_emb_dict, user_emb_dict, news_emb_dict):
        ...
        if node_type == "news":
            ...
            attention_out_image, self.hidden_image = self.attention_image(linear_input_image, linear_input_image,
                                                                          linear_input_image)
            attention_out_time, self.hidden_time = self.attention_time(linear_input_time, linear_input_time,
                                                                       linear_input_time)
        ...
        if node_type == "post":
            ...
            attention_out_text, self.hidden_text = self.post_content_attention_text(linear_input_text,
                                                                                    linear_input_text,
                                                                                    linear_input_text)
            attention_out_other, self.hidden_other = self.post_content_attention_other(linear_input_other,
                                                                                       linear_input_other,
                                                                                       linear_input_other)
            attention_out_time, self.hidden_time = self.attention_time(linear_input_time, linear_input_time,
                                                                       linear_input_time)
    ...
    def load_entropy_data(self, file_path='../entropy/entropy_values.json'):
        # 加载每篇新闻的熵信息
        with open(file_path, 'r') as f:
            entropy_dict = json.load(f)
        return entropy_dict

    def caculate_weighted_entropy(self, news_id):
        if news_id in self.entropy_data:
            entropy_value = self.entropy_data[news_id]
            entropy_weight = F.softmax(torch.tensor(entropy_value), dim=0)
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, all_t0, npu=30):
        ...
        entropy_weight = self.caculate_weighted_entropy(het_node.node_id)

        node_embedding = []
        c_agg_batch = self.Bi_RNN([het_node.node_id], [0], het_node.node_type, post_emb_dict, user_emb_dict,
                                  news_emb_dict)
        node_embedding.append(c_agg_batch[0] * entropy_weight)
        node_embedding_n = []
        ...
        self_att_g = self.mh_attention1(graph_feature.view(bsz, -1, self.hidden_dim),
                                        graph_feature.view(bsz, -1, self.hidden_dim),
                                        graph_feature.view(bsz, -1, self.hidden_dim))
        self_att_c = self.mh_attention1(node_embedding_n.view(bsz, -1, self.hidden_dim),
        ...
        decoder_emb_input += self.type_encoder(decoder_type_input).view(decoder_emb_input.shape[0], 1,
                                                                        decoder_emb_input.shape[2])
...
for repeat in range(5):
    PATH = base_PATH + 'lr' + str(lr) + 'bs' + str(batch_size) + 'od' + str(output_dim) + 'hd' + str(
        hidden_dim) + 'al' + str(alp) + 'be' + str(belt) + '/repeat' + str(repeat) + '/'
    print("PATH:", PATH)
    ...
    for fold in range(num_folds):
        ...
        best_epoch = 0
        start_epoch = 0
        net = DTN(input_dim=[768, 512, 3, 29],
        ...
        for epoch in range(start_epoch, num_epoch):
            ...
            for i in range(len(train_set)):
                ...
                node_id = train_set[i].node_id
                if node_id in neighbor_dict[2]:
                    all_t.extend(neighbor_dict[2][node_id])

                if node_id in neighbor_dict[0]:
                    output, dist_og = net(train_set[i], neighbor_dict[0][node_id], all_t)
                    out = output.item()
                else:
                    continue
                if (out / (1 - out) >= ratio and train_set[i].label == 1) or (
                    ...
                    c += 1
                # cur_loss = BCELoss(predictions=output, true_label=train_set[i].label,weight_real=1.0, weight_fake=20)
                ...
                if i % 100 == 99:
                    ...
                    with open(f_path, "a") as f:
                        f.write('Repeat: %d,Fold: %d, Epoch: %d, step: %5d, loss: %.4f,loss_dis: %.4f,  acc: %.4f \n' %
                                (repeat + 1, fold + 1, epoch + 1, i + 1, running_loss / 100, running_loss_dis / 100,
                                 c / 100))
                    running_loss = 0.0
                    running_loss_dis = 0.0
                    c = 0.0
            ...
            for j in range(len(val_set)):
                ...
                all_t = []
                all_t.append(n_add_time_dict[val_set[j].node_id])

                node_id = val_set[j].node_id
                if node_id in neighbor_dict[2]:
                    all_t.extend(neighbor_dict[2][node_id])

                if node_id in neighbor_dict[0]:
                    output, dist_og = net(val_set[j], neighbor_dict[0][node_id], all_t)
                    out = output.item()
                    valid_count += 1
                else:
                    continue
            ...
            if fake_precision + fake_recall:
                ...
                fake_f1 = 0
            print('Validation loss: %.4f, Validation loss_dis: %.4f,Validation accuracy: %.4f' % (
                val_loss / valid_count, val_loss_dis / valid_count, val_acc))
    ...
    for k in range(len(test_set)):
        ...
        for fold in range(num_folds):
            all_t = []
            node_id = test_set[k].node_id

            if node_id in n_add_time_dict:
                all_t.append(n_add_time_dict[node_id])
            else:
                continue
            ...
            if node_id in neighbor_dict[0]:
                result, dist_og = best_models[fold](test_set[k], neighbor_dict[0][node_id], all_t)
                output += result.item()
                valid_test_count += 1
            else:
                continue
    ...
    test_accuracy = t / valid_test_count


    real_precision = real_true / (real_true + fake_count - fake_true) if (real_true + fake_count - fake_true) > 0 else 0
    fake_precision = fake_true / (fake_true + real_count - real_true) if (fake_true + real_count - real_true) > 0 else 0
    real_recall = real_true / real_count if real_count > 0 else 0
    fake_recall = fake_true / fake_count if fake_count > 0 else 0
    real_f1 = 2 * real_precision * real_recall / (real_precision + real_recall) if (
                                                                                           real_precision + real_recall) > 0 else 0
    fake_f1 = 2 * fake_precision * fake_recall / (fake_precision + fake_recall) if (
                                                                                           fake_precision + fake_recall) > 0 else 0
    print('Test loss: %.4f, Test loss_dis: %.4f, Test accuracy: %.4f' % (
        test_loss / valid_test_count, test_loss_dis / valid_test_count, test_accuracy))
    print('Real Precision: %.4f, Real Recall: %.4f, Real F1: %.4f' % (real_precision, real_recall, real_f1))
    print('Fake Precision: %.4f, Fake Recall: %.4f, Fake F1: %.4f' % (fake_precision, fake_recall, fake_f1))
    ...
    res = []
    res.append(round((test_loss / valid_test_count), 4))
    res.append(round((test_loss_dis / valid_test_count), 4))
    res.append(round(test_accuracy, 4))

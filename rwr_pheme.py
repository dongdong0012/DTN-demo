...
def rwr_worker(start_node, nei_list_subsets, desc, j, nodes_len, typed_rwr, no_rwr, max_steps, max_neigh, restart_rate, adj_list,node_types,min_neigh,max_uniq_neigh,edges_to_enforce):
...
def random_walk_with_restart(node_types,min_neigh,max_uniq_neigh,edges_to_enforce):
    nei_list, nodes = dict(), dict()
    nodes = {t : set() for t in node_types}
    involved = {t : set() for t in node_types}
    manager = Manager()
    ...
    def rwr(nodes_set, desc):
        ...
        with Pool(num_process) as p:
            p.starmap(rwr_worker, [(nodes_list[i], nei_list_subsets, desc, i, len(nodes_list), typed_rwr, no_rwr, max_steps, max_neigh, restart_rate, adj_list,node_types,min_neigh,max_uniq_neigh,edges_to_enforce) for i in range(len(nodes_list))])
...
if __name__ == "__main__":
    ...
    assert sum([typed_rwr, sensitivity_test, no_rwr]) <= 1
    ...
    if typed_rwr:
        ...
        max_uniq_neigh = {node_type: 100 if node_type == 'u' else 5 for node_type in node_types}
        configuration_tag = prefix + '_'.join([f'{k}{max_uniq_neigh[k]}' for k in node_types])
    elif no_rwr:
        configuration_tag = prefix + 'norwr'
    ...
    random_walk_with_restart(node_types, min_neigh, max_uniq_neigh, edges_to_enforce)

    adj_list1 = dict()
    for k, v in adj_list.items():
        adj_list1[k.split("t")[0]] = [i.split("t")[0] for i in v]
    with open(output_dir + "/original_adj", 'w') as f:
        json.dump(adj_list1, f)

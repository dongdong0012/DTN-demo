...
def embed_text(ids, texts, max_seq_len, config, dir_name, num_process = 2):  # 8
    def skip_existed(id_list, text_list, dir_name):
        print('skip existed and empty...')
        dn = os.path.join(out_dir, dir_name).replace('\\', '/')
        existed = set([fn for fn in os.listdir(dn) if
                       os.stat(os.path.join(dn, fn).replace('\\', '/')).st_size > 0]) if os.path.isdir(dn) else set()
    ...
    def embed_text_sequential(texts):
        ...
        model_path = 'E:/model/bertweet-base'
        embedder = TextEmbedder(max_seq_len, config['model name'],model_path,device=device)
...
def process_tweets():
    ...
    with open(os.path.join(involved_dir, 'n_involved.txt'), 'r') as f:
        involved_ids_addtime = set(f.readlines()[0].strip().split(' '))
    with open(os.path.join(involved_dir, 'p_involved.txt'), 'r') as f:
        involved_ids_addtime = involved_ids_addtime.union(set(f.readlines()[0].strip().split(' ')))

    involved_ids = set(id[:-11] for id in involved_ids_addtime)
    ...
    for event_raw in listdir(in_dir):
        ...
        for rumority in ['non-rumours', 'rumours']:
            for news_id in tqdm(listdir(join(in_dir, event_raw, rumority)),
                                desc=f'{event}-{rumority}'):
                if news_id == '.DS_Store': continue
                for folder in ['source-tweet', 'reactions']:
...
def process_user_description(num_process = 2):
    print('process_user_description')
    manager = Manager()
    with open(os.path.join(involved_dir, 'u_involved.txt'), 'r') as f:
        involved_uids_addtime = set(f.readlines()[0].strip().split(' '))
    # 提取实际的 involved_uids 部分
    involved_uids = set(id[:-2] for id in involved_uids_addtime)
    for event_raw in listdir(in_dir):
        if event_raw[-16:] != '-all-rnr-threads': continue
        for rumority in ['non-rumours', 'rumours']:
